...
@torch.no_grad()
def rollout_relabel(student, teacher, envs, n_ep, M, recN, H_s, H_t, A, n_evaders, device, beta=0.0, rng=None):
    ...
    while len(episodes) < n_ep:
        ...
        for i in np.flatnonzero(dones_env):
            ...
            ep_obs[i] = []
            ep_lp[i] = []
    return episodes, (np.mean(caps) / n_evaders if caps else 0.0)
...
def main(argv):
    ...
    for r in range(1, all_args.dagger_rounds + 1):
        ...
        fresh, catch = rollout_relabel(student, teacher, envs, all_args.episodes_per_round,
                                       M, recN, H_s, H_t, A, all_args.n_evaders, device, beta=beta, rng=rng)
        buffer.extend(fresh)
