...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        # value-normalizer lives on the trainer (created above), so restore it here
        # rather than in restore(), which runs before the trainer exists.
        if self.model_dir is not None:
    ...
    def save(self, episode=0):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            # Persist optimizer + value-normalizer state so a run can be resumed
            # cleanly (without a warm-start transient). PopArt's normalizer is the
            # critic's v_out, already saved inside critic.pt above.
    ...
    def restore(self, model_dir):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            if not self.all_args.use_render:
                ...
                # Restore optimizer state if present. Guarded by existence so old
                # checkpoints (actor/critic only) still load without error.
    ...
    def restore_value_normalizer(self, model_dir):
        ...
        Guarded by existence for backward compatibility with old checkpoints,
