...
class GMMPrior(nn.Module):
    ...
    def mean_repulsion(self, eps: float = 1e-2) -> torch.Tensor:
        """Log-barrier repulsion between component means (mu is (K, D) for both
        structures): -sum_{i<j, d} log(|mu_i_d - mu_j_d| + eps). Positive, and its
        gradient pushes adjacent means apart; add `tau * repulsion` to the loss to
        encourage mean separation."""
