...
class IdentityChannel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.delta = delta
        # Share the delta-learnability interface so callers needn't special-case "none".
...
class DDCL_Async_SD(nn.Module):
    """Like DDCL_SD, but the receiver subtracts a freshly-sampled eps2 instead of the
    sender's eps1. Breaks the exact theoretical guarantee but lets the receiver run
    independently of the sender (hence "async")."""
